...
diagnostic_AQ_plot <- function(curve_data, fit_data, Photo, PARi, group_id){
      ...
      for(i in seq_along(1:length(unique(curve_data[[group_id]])))){
            single_curve <- curve_data[curve_data[[group_id]] == unique(curve_data[[group_id]])[i],]
            plot(
                  ...
                  pch = 3,
                  cex = 2,
                  xlab = expression("PARi (µmol photons "*m^-2*" "*s^-1*")"),
                  ylab = expression("Photo (µmol photons "*m^-2*" "*s^-1*")"),
